...
public class VMEventManager
{
    ...
    public void dispatchEvent(@NotNull VMEvent event)
    {
        ...
        catch (NoSuchFieldException | IllegalAccessException e)
        {
            throw new IllegalArgumentException(
                    "Failed to get handler list for event: " + event.getClass().getName(),
                    e
            );
        }
            ...
            for (Method method : listener.getClass().getDeclaredMethods())
            {
                ...
                eventHandlerList.registerHandler(
                        event -> {
                            ...
                            {
                                throw new IllegalArgumentException(
                                        "Failed to invoke method " + method.getName() + " in listener " + listener.getClass()
                                                                                                                  .getName() + " with event " + event.getClass()
                                                                                                                                                     .getName(),
                                        e
                                );
                            }
    ...
    private static @NotNull VMEventHandlerList<?> getHandlerListForEvents(@NotNull VMListener listener,
                                                                          Method method) throws NoSuchFieldException, IllegalAccessException
    {
        if (method.getParameterCount() != 1)
            throw new IllegalArgumentException("Method listener " + listener.getClass()
                                                                            .getName() + " must have exactly one parameter.");
        ...
        if (!eventHandlerList.getEventType().isAssignableFrom(eventType))
            throw new IllegalArgumentException("Field HANDLER_LIST in listener " + listener.getClass()
                                                                                           .getName() + " must be of type VMEventHandlerList<" + eventType.getName() + ">.");
        return eventHandlerList;
    }

    private static @NotNull VMEventHandlerList<?> getHandlerListForEvent(
            Class<?> eventType) throws NoSuchFieldException, IllegalAccessException
    {
